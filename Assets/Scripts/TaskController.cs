...
/*
    ...
    private void DecideCursorPos()
    {
        if(device == Device.Pen)
        {
            mouseX = (tablet.x*2) * 1920 - 1920/2;
            mouseY = tablet.y * 1080 - 1080/2;
            print(tablet.y);
        }
        if(device == Device.Mouse)
        {
            mouseX = Input.mousePosition.x - 1920/2;
            mouseY = Input.mousePosition.y - 1080/2;
        }
    ...
    private void DrawCursorTrajestory()
    {
        if(Touching)
        {
            ...
            trajestoryList.Add(new Vector3(mouseX, mouseY, 1));
    ...
    private void taskUpdate()
    {
        ...
        start.transform.localScale = new Vector3(nowTaskWidth,3,1);
        goal.transform.localScale = new Vector3(nowTaskWidth,3,1);
        start.transform.position = new Vector3((nowTaskAmplitude/2)-nowTaskWidth/2,-1,0);
        goal.transform.position = new Vector3(-(nowTaskAmplitude/2)-nowTaskWidth/2,-1,0);
    }
    //マウス座標を保存するCSVを作成
